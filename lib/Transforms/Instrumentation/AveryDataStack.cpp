...
/// be safe.
class DataStack : public FunctionPass {
...
public:
  ...
  DataStack(const TargetMachine *TM)
...
uint64_t DataStack::getStaticAllocaAllocationSize(const AllocaInst* AI) {
...
bool DataStack::IsAccessSafe(Value *Addr, uint64_t AccessSize,
                             const Value *AllocaPtr, uint64_t AllocaSize) {
...
bool DataStack::IsMemIntrinsicSafe(const MemIntrinsic *MI, const Use &U,
                                   const Value *AllocaPtr,
                                   uint64_t AllocaSize) {
...
bool DataStack::IsSafeStackAlloca(const Value *AllocaPtr, uint64_t AllocaSize) {
...
void DataStack::findInsts(Function &F,
...
AllocaInst *
DataStack::createStackRestorePoints(IRBuilder<> &IRB, Function &F,
                                    ArrayRef<Instruction *> StackRestorePoints,
                                    Value *StaticTop, bool NeedDynamicTop) {
...
Value *DataStack::moveStaticAllocasToUnsafeStack(
...
void DataStack::moveDynamicAllocasToUnsafeStack(
...
bool DataStack::runOnFunction(Function &F) {
...
char DataStack::ID = 0;
INITIALIZE_TM_PASS_BEGIN(DataStack, "data-stack",
                         "Data Stack instrumentation pass", false, false)
INITIALIZE_TM_PASS_END(DataStack, "safe-stack",
                       "Data Stack instrumentation pass", false, false)

FunctionPass *llvm::createDataStackPass(const llvm::TargetMachine *TM) {
  return new DataStack(TM);
}
